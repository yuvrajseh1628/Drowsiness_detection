...
while qwertyu:
    ...
    for (x, y, w, h) in faces:
        ...
        # If the eyesDrowsy, start counting
        if status1 == 2 and status2 == 2:
        #if pred1 == 2 and pred2 == 2:
            count += 1
            cv2.putText(frame, "Drowsy, Frame count: " + str(count), (10, 30), cv2.FONT_HERSHEY_COMPLEX, 1, (0, 0, 255), 1)
            # if eyesDrowsy for 10 consecutive frames, start the alarm
            if count >= 3:
            ...
            cv2.putText(frame, "Awake", (10, 30), cv2.FONT_HERSHEY_COMPLEX, 1, (0, 255, 0), 1)
            count = 0
            alarm_on = False
